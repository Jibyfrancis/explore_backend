...
export const seachProperty = async (data: any, userRepository: ReturnType<UserDbInterface>) => {
  return await userRepository.searchingProperty(data)
}
export const deletePropertyType = async (id: Types.ObjectId, userRepository: ReturnType<UserDbInterface>, cloudService: ReturnType<CloudServiceInterface>) => {
  const property: CreatePropertyInterface[] | null = await userRepository.findPropertyById(id)
  const imageUrls: string[] =[...property[0].imageUrl]
  await cloudService.deleteMultiples(imageUrls)
  const removeProperty=await userRepository.deleteProperty(id)
  return removeProperty
 

}
export const updateProperty=async(id:Types.ObjectId,userRepository:ReturnType<UserDbInterface>,data:CreatePropertyInterface)=>{
